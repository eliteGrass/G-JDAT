package com.itsoku.lesson037.mq.sender.retry;

import com.itsoku.lesson037.mq.enums.DelayLevelEnums;
...
public class DefaultMqSendRetry implements MqSendRetry {
    ...
    @Override
    public MqSendRetryResult getRetryResult(MsgPO msgPO) {
        MqSendRetryResult mqSendRetryResult = new MqSendRetryResult();
        //当前失败次数 < 最大重试次数，则可以继续重试
        if (msgPO.getFailCount() < MAX_RETRY_COUNT) {
            //获取延迟时间
            long delayTimeInMills = this.getDelayTimeInMills(msgPO.getFailCount());
            //下次重试时间（当前时间+延迟时间）
