package com.itsoku.lesson036.mq.sender;

import cn.hutool.core.exceptions.ExceptionUtil;
import cn.hutool.core.util.IdUtil;
import cn.hutool.json.JSONUtil;
import com.itsoku.lesson036.lock.DistributeLock;
import com.itsoku.lesson036.mq.delay.DelayTaskProcessor;
import com.itsoku.lesson036.mq.dto.Msg;
import com.itsoku.lesson036.mq.enums.MsgStatusEnum;
import com.itsoku.lesson036.mq.po.MsgPO;
import com.itsoku.lesson036.mq.properties.MqProperties;
import com.itsoku.lesson036.mq.sender.retry.MqSendRetry;
import com.itsoku.lesson036.mq.sender.retry.MqSendRetryResult;
import com.itsoku.lesson036.mq.service.IMsgService;
import com.itsoku.lesson036.mq.service.ISequentialMsgNumberGeneratorService;
import com.itsoku.lesson036.utils.CollUtils;
...
public class DefaultMsgSender implements IMsgSender {
    ...
    public DefaultMsgSender(MqProperties mqProperties,
                            ...
                            ISequentialMsgNumberGeneratorService sequentialMsgNumberGeneratorService,
        ...
        this.sequentialMsgNumberGeneratorService = sequentialMsgNumberGeneratorService;
        this.transactionTemplate = transactionTemplate;
    }

    @Override
    public <T> Msg<T> build(T msgBody) {
        return Msg.msg(this.mqProperties.getProducer(), IdUtil.fastSimpleUUID(), msgBody);
    }

    @Override
    public void send(String exchange, String routingKey, List<Msg<?>> msgList) {
        this.send(exchange, routingKey, LocalDateTime.now(), msgList);
    }

    @Override
    public void send(String exchange, String routingKey, long delayTime, TimeUnit delayTimeUnit, List<Msg<?>> msgList) {
        if (delayTime <= 0) {
        ...
        //发送消息
        this.send(exchange, routingKey, expectSendTime, msgList);
    }

    private void send(String exchange, String routingKey, LocalDateTime expectSendTime, List<Msg<?>> msgList) {
        ...
        if (this.isNeedStoreToDb(expectSendTime, msgList)) {
            //有事务，则先入库
            List<MsgPO> msgPOList = this.msgService.batchInsert(exchange, routingKey, expectSendTime, msgBodyJsonList);
            ...
            for (String msgBodyJson : msgBodyJsonList) {
                this.deliverMsgToMq(exchange, routingKey, msgBodyJson);
            }
        }
    }
     ...
     * @return
     */
    private boolean isNeedStoreToDb(LocalDateTime expectSendTime, List<Msg<?>> msgList) {
        //事务消息 || 延迟消息
    ...
    private void deliverImmediateMsg(String msgId) {
        ...
        this.distributeLock.accept("deliverImmediateMsg:" + msgId,
                lockKey -> {
                    ...
                    try {
                        //投递消息到MQ
                        deliverMsgToMq(msgPO.getExchange(), msgPO.getRoutingKey(), msgPO.getBodyJson());
                    } catch (Exception e) {
                        exception = e;
                        log.error("投递消息失败:{}", e.getMessage(), e);
                    }
                    //没有异常，说明消息投递成功
                    if (exception == null) {
                        //将消息状态置为成功
    ...
    private void deliverMsgToMq(String exchange, String routingKey, String msgBodyJson) {
        ...
        log.info("消息已投递到rabbitmq,exchange:{},routingKey:{}", exchange, routingKey);
    }
