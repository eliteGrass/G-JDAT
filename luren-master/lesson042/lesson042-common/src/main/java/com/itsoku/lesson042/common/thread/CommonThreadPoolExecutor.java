package com.itsoku.lesson042.common.thread;
 ...
 */
public class CommonThreadPoolExecutor extends ThreadPoolExecutor implements DisposableBean {
    private ThreadLocalContext threadLocalContext;

    public CommonThreadPoolExecutor(ThreadLocalContext context, int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit, BlockingQueue<Runnable> workQueue) {
    ...
    private class CallableWrap<V> implements Callable<V> {
        ...
        public CallableWrap(Callable<V> target) {
            this.target = target;
            this.context = CommonThreadPoolExecutor.this.threadLocalContext.getContext();
        }

        @Override
        public V call() throws Exception {
            CommonThreadPoolExecutor.this.threadLocalContext.setContext(this.context);
