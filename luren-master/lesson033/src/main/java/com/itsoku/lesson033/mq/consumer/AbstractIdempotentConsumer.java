package com.itsoku.lesson033.mq.consumer;

import cn.hutool.core.exceptions.ExceptionUtil;
import cn.hutool.json.JSONUtil;
import com.itsoku.lesson033.mq.consumer.retry.MsgConsumeRetry;
import com.itsoku.lesson033.mq.consumer.retry.MsgConsumeRetryResult;
import com.itsoku.lesson033.mq.dto.Msg;
import com.itsoku.lesson033.mq.enums.MsgConsumerStatusEnum;
import com.itsoku.lesson033.mq.po.MsgConsumePO;
import com.itsoku.lesson033.mq.sender.IMsgSender;
import com.itsoku.lesson033.mq.service.IMsgConsumerService;
import com.itsoku.lesson033.mq.service.IdempotentService;
 ...
 */
@Slf4j
public abstract class AbstractIdempotentConsumer<B, M extends Msg<B>> {
    ...
    public void dispose(Message message) {
        ...
        try {
            ...
            this.idempotentService.idempotent(idempotentKey, () -> {
                //调用子类的方法，消费消息
                this.disposeIn(message, msg.getBody());
            });
            //消费成功，记录日志
            this.consumeSuccess(msgConsumerPO);
        } catch (Exception exception) {
            log.error("消费记录：{}", msgConsumerPO);
     ...
     * @return
     */
    private MsgConsumePO createMsgConsumePO(Message message, Msg<B> msg) {
     ...
     * @param exception
     */
    private void consumeFail(Message message, Msg<B> msg, MsgConsumePO msgConsumerPO, Exception exception) {
        //获取消费重试信息
    ...
    protected Msg<B> getMsg(Message message) {
        try {
     ...
     * @param body    消息体
     */
    protected abstract void disposeIn(Message message, B body);
}
